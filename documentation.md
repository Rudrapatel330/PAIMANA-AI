...
│   ├── chat.py                 # LLM Chatbot module (Groq + RAG-lite)
│   └── ml/
...
│   │       └── ChatAssistant.jsx # LLM Conversational Interface
...
| g. AI-powered Monitoring Dashboard | Full React dashboard with 6 pages | Frontend (`/`, `/predict`, `/risk`, `/alerts`, `/analytics`, `/chat`) |
| h. LLM-enabled Project Intelligence | Conversational AI using Groq API (Llama 3) | `/chat` page + `chat.py` + `/api/chat` endpoint |
| i. Documentation & Deployment | This document + Docker Compose framework | `documentation.md`, `docker-compose.yml`, `Dockerfile` |
...
The entire application (frontend + backend) is containerized and orchestrated via Docker Compose.

```bash
cd 0SIHNEW
# Setup environment variables (add Groq API key for Chatbot)
cp .env.example .env
